...
CST_colors <- c("1" = "#f2ad00", "2" = "#713030", "3" = "#82a1c1", "4" = "#018855", "5" = "#aa4365","6" = "#cd6677","7" = "#494978","8" = "#59a77f")
...
frequency_plot = ggplot(frequency_melt, aes(x = Species, y = value)) +
  geom_violin(scale = "width", fill = "white") +
  geom_point(alpha = 0.3, fill = "lightgrey")+
...
```{r}
count_diversity_CS %>% summarise(p_Richness = kruskal.test(Richness~population)$p.value, 
...
population_grid_CS = plot_grid(population_nmds_CS, population_diversity_CS,ncol = 1,labels = c("B","C"),label_size = 18)
population_grid2_CS = plot_grid(frequency_plot, population_grid_CS,nrow = 1,labels = c("A",""), label_size = 18)
population_grid2_CS
```

Save figure
```{r}
#save_plot("output/Figure2.pdf", population_grid2_CS, ncol = 2, nrow = 2.4)
```
...
prev_table <- df_aureus %>% group_by(population) %>% 
  summarise(S.aureus_absent_n = matrixStats::count(Staphylococcus_aureus == 0) , S.aureus_present_n= matrixStats::count(Staphylococcus_aureus>0)) %>% 
 group_by(population)  %>% 
  summarise("S.aureus_present >0%" = paste0(S.aureus_present_n, " (", round(S.aureus_present_n/sum(S.aureus_absent_n, S.aureus_present_n)*100),"%)"),
...
prev_table10 <- df_aureus %>% group_by(population) %>% 
  summarise(S.aureus_absent_n = matrixStats::count(Staphylococcus_aureus <10) , S.aureus_present_n= matrixStats::count(Staphylococcus_aureus>=10)) %>% 
 group_by(population)  %>% 
  summarise("S.aureus_present >=10%" = paste0(S.aureus_present_n, " (", round(S.aureus_present_n/sum(S.aureus_absent_n, S.aureus_present_n)*100),"%)"),
...
df_aureus2 <- as.data.frame(t(count_table[,colnames(count_table) %in% df_aureus$Sample])) %>% select(-Staphylococcus_aureus) %>%
  ...
  rowwise(Sample) %>%  summarise(Other_Staph=sum(c_across(where(is.numeric))))  %>% 
  left_join(df_aureus) %>% 
...
theme(axis.title.x = element_blank(), axis.text.x = element_blank(), axis.ticks.x = element_blank(), legend.position = c(0.1, 0.75), legend.title = element_blank()) + 
labs(y = "Relative abundance [%]") + ggtitle("Germany") + 
...
labs(y = "Relative abundance [%]") + ggtitle("Denmark")+ 
geom_hline(yintercept = 10, color="red", size=0.5)+ 
...
#save_plot("output/FigureS3.pdf", SA_pres, ncol = 1.5, nrow = 1.5)
```
...
#pdf(file = "output/Figure3.pdf", width=10, height=7) #uncomment to save file

corrplot(M3, method ="color", type="lower",  is.corr = FALSE,
         p.mat=as.matrix(testRes_sign2),sig.level = c(0.001), pch.cex = 0.65,insig = 'label_sig', pch = c("p<0.001"), pch.col = 'black', col.lim= c(-1,1),col= colorRampPalette(c("#18397a", "white", "#980505"))(11), addgrid.col = 'grey')
...
```{r}
count_summary = count_table3 %>% group_by(sCST) %>% summarise(aureus = mean(Staphylococcus_aureus), epidermidis = mean(Staphylococcus_epidermidis), capitis = mean(Staphylococcus_capitis), lugdunensis = mean(Staphylococcus_lugdunensis), hominis = mean(Staphylococcus_hominis), CoNS = mean(CoNS), Other_Staph = mean(Other_Staph)) %>% mutate_at(2:8, ~(round(., digits = 1)))
...
Save figure  
```{r}
#save_plot("output/Figure4.pdf", F4.1, ncol = 2, nrow = 3.7)
```
...
Save figure
```{r}
#save_plot("output/Figure6A.pdf",time3,ncol = 0.9, nrow = 1.8)
```
...
Save figure
```{r}
#pdf(file = "output/Fig6B_Markov3.pdf")
#plotmat(as.matrix(t(markov_12_round)), pos = c(2), lwd = 1.5, self.lwd = 1.5,box.lwd = 1, cex.txt = 0.7, box.size = 0.1, box.type = "circle", box.prop = 1, box.col = c("#e68502", "#82a1c1"), arr.length = 0.15, arr.width = 0.15, self.cex = 0.54, self.shifty = c(0,0), self.shiftx = c(-0.14,0.14), shadow.size=0, arr.type = "triangle")
#dev.off()
```
...
Save figure
```{r}
#pdf(file = "output/Fig6C_Markov3.pdf")
#plotmat(as.matrix(t(markov_aureus_round)), pos = c(1,2), lwd = 1.5, self.lwd= 1.5, box.lwd = 1, cex.txt = 0.7, box.size = 0.1, box.type = "circle", box.prop = 1, box.col = c("#e68502","#fad77b", "#82a1c1"),  arr.length = 0.15, arr.width = 0.15, self.cex = 0.54, self.shifty = c(0.14,0,0), self.shiftx = c(0,-0.14,0.14), shadow.size=0, arr.type = "triangle")
#dev.off()
```
...
Save figure
```{r}
#pdf(file = "output/Fig6D_Markov3.pdf")
#plotmat(as.matrix(t(markov_Cons_round)), pos = c(2,2), lwd = 1.5, self.lwd = 1.5, box.lwd = 1, cex.txt = 0.7, box.size = 0.1, box.type = "circle", box.prop = 1, box.col = c("#f2ad00","#ac8cae","#82a1c1", "#a7bf90"), arr.length = 0.15, arr.width = 0.15, self.cex = 0.54, self.shifty = c(0,0,0,0), self.shiftx = c(-0.14,0.14,-0.14, 0.14), shadow.size=0, box.cex = 0.9, arr.type = "triangle")
#dev.off()
```
...
count_metadata_sample_time_long %>% group_by(time_dist) %>% summarise(mean = mean(days_difference), median = median(days_difference), min = min(days_difference), max = max(days_difference)) %>% kable() %>% kable_classic()
```
...
Save figure
```{r}
#save_plot("output/FigureS5.pdf",average_plot,ncol = 0.9, nrow = 1.2)
```
...
```{r}
staph_median = count_metadata2_abs  %>% group_by(sCST) %>% summarise(percentage_median = median(staph_percentage))
...
#Add sCST count
CST_count <- count_metadata2_abs  %>% group_by(sCST) %>% summarise(count=n())
...
qPCR_plot = ggplot(count_metadata2_abs2_melt, aes(x = sCST, y = value)) +
  geom_boxplot(aes(fill = variable), alpha = 1, outlier.alpha = 1, position = position_dodge2(width = 0.5)) +
  scale_fill_manual(name = "qPCR",
                    ...
                    values = c("#595959","grey","#f2ad00","#82a1c1")) +
  theme_cowplot()+
  ...
  theme(legend.position = "bottom", legend.key.size = unit(1.2, "cm"),legend.text.align = 0,axis.title.x = element_blank()) +
...
qPCR_grid = plot_grid(qPCR_plot,staph_percentage_plot,ncol = 1, align = "v",rel_heights = c(5,4), labels = "AUTO",label_size = 16)
qPCR_grid
```

Save figure
```{r}
#save_plot("output/Figure8.pdf", qPCR_grid,nrow = 2.4,ncol = 1.8)
```
...
### Plot percentage of bacterial CSTs within each sCST  
```{r fig.height=5, fig.width=12}
group_sCST = ggplot(data = count_na_tally %>% mutate(bacterial_CST = as.factor(bacterial_CST)), aes(x = sCST, y = n, fill = bacterial_CST)) +
  geom_bar(position = "fill", stat = "identity") +
  scale_fill_manual(values= CST_colors, labels = c("CST1 - S. aureus","CST2 - Enterobacteriaceae \n     & Bacillus spp.","CST3 - S. epidermidis","CST4 - High Cutibacterium spp.","CST5 - Corynebacterium spp.","CST6 - Moraxella spp.","CST7 - Dolosigranulum sp.","CST8 - Cutibacterium spp.")) +
  theme_cowplot() +
...
### Plot percentage of sCSTs within each bacterial CST 
```{r fig.height=5, fig.width=12}
group_CST = ggplot(data = count_na_tally, aes(x = bacterial_CST, y = n, fill = sCST)) +
  geom_bar(position = "fill", stat = "identity") +
  scale_fill_manual(values=sCST_colors) +
  theme_cowplot() +
  ylab("Percentage of samples") +
  xlab("") +
  scale_x_discrete(labels= c("1" = "CST1", "2" = "CST2", "3" = "CST3", "4" = "CST4", "5" = "CST5", "6" = "CST6", "7" = "CST7", "8" = "CST8")) +
  scale_y_continuous(expand = c(0,0), labels = scales::label_percent()) +
  theme(legend.position = "right")
group_CST
```
...
h1 = plot_grid(group_CST, group_sCST, align = "hv", ncol = 1, labels = "AUTO", label_size = 20)
h1
```

Save figure
```{r}
#save_plot("output/Figure7.pdf", h1, nrow = 2, ncol = 1.9)
```
...
removed_summary = count_temp_removed %>% group_by(bacterial_CST) %>% tally() %>%
...
my.fill <- RColorBrewer::brewer.pal(8,"Set3")
...
Save figure
```{r}
#save_plot("output/FigureS6.pdf", removed_CST,nrow = 1,ncol = 1.8)
```
...
### Distribution of populations (DK and GER) within the two major groups (sCST1 and sCST2)   
Test whether the two major sCST groups differ with regards to distribution of populations   
```{r}
meta_pop <- count_metadata2 %>% mutate(sCST_main = ifelse(sCST %in% c("sCST1Sa_H", "sCST1Sa_M"), "sCST1", "sCST2")) 

chisq.test(table(meta_pop$population,meta_pop$sCST_main))
```

#### Calculate percentages and 95% CI of population distribution within the two major sCST groups  
```{r}
meta_pop <- meta_pop %>% group_by(population, sCST_main) %>% tally() %>% group_by(population) %>% mutate(total_per_population = sum(n), perc = round((n/total_per_population)*100,1))
...
meta_pop$row.no<-1:nrow(meta_pop)
df_ci<-data.frame()
n<-1:nrow(meta_pop)
for(i in n){
  ci_low<-round2(binom.test(meta_pop$n[i],meta_pop$total_per_population[i])$conf.int[1]*100,1)
  ci_hi<-round2(binom.test(meta_pop$n[i],meta_pop$total_per_population[i])$conf.int[2]*100,1)
  row.no<-i
...
#### Calculate percentage of Danish and German samples in each sCST normalized for a differing number of samples in each of the two populations    
```{r fig.height=5, fig.width=9, warning=FALSE}
metadata_tally = na.exclude(count_metadata2 %>% group_by(sCST, population) %>% tally())
metadata_tally = metadata_tally %>% group_by(population) %>% mutate(normalise_n = n/sum(n)) %>% group_by(sCST, population) %>% summarise(normalise_n = sum(normalise_n)) %>% mutate(percentage = round(normalise_n/sum(normalise_n)*100, digit = 1))
metadata_tally %>% kable() %>% kable_classic()
```

#### Test whether the sCSTs differ with regards to distribution of populations  
```{r fig.height=5, fig.width=9, warning=FALSE}
chisq.test(count_metadata2$sCST,count_metadata2$population)
```

### Plot distribution of populations in the sCSTs   
```{r fig.height=5, fig.width=12, warning=FALSE}
population_plot = ggplot(metadata_tally, aes(x = sCST, y = normalise_n, fill = population)) +
  geom_bar(position = "fill", stat = "identity") +
...
metadata_tally = na.exclude(count_metadata2 %>% group_by(population, sCST, sex) %>% tally())
metadata_tally = metadata_tally %>% group_by(population,sex) %>% mutate(normalise_n = n/sum(n)) %>% group_by(population, sCST) %>% mutate(percentage = round(normalise_n/sum(normalise_n)*100, digit =1))
metadata_tally %>% kable() %>% kable_classic()
```


### Plot distribution of sexes in the sCSTs in the two populations  
```{r fig.height=7, fig.width=12, warning=FALSE}
...
meta1_grid = plot_grid(population_plot, sex_plot,ncol = 1, align = "hv", labels = "AUTO", label_size = 18,rel_heights = c(1,2))
meta1_grid
#save_plot("output/Figure5.pdf", meta1_grid,ncol = 1.4,nrow = 2.1)
```
...
Save figure
```{r}
#save_plot("output/FigureS2.pdf", population_dist, ncol = 1.3, nrow = 1.4)
```
...
count_metadata_test_age %>% group_by(population) %>% summarise(median(age_group, na.rm = T), min(age_group, na.rm = T), max(age_group, na.rm = T)) %>% kable() %>% kable_classic()
...
smoking_age_plot <- plot_grid(age_plot_alt2,smoker_plot,align = "hv",ncol = 1,labels = "AUTO",label_size = 18)

smoking_age_plot

#save_plot("output/FigureS4.pdf",smoking_age_plot,nrow = 2, ncol = 1.6)
```
...
Combine plot and save figure
```{r fig.height=10, fig.width=12}
carrier_number = plot_grid(carrier_plot, carrier_plot2, align = "hv", ncol = 1, labels = "AUTO")
carrier_number
#save_plot("output/Figure9.pdf", carrier_number,nrow = 1.7, ncol = 2)
```
